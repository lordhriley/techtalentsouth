...
engine = sqlalchemy.create_engine('mssql+pyodbc://sa:Carito33156@VDEV003/Qualex_Stage_SM?driver=ODBC+Driver+17+for+SQL+Server')
    ...
    def load_data_to_sql(self):
        if self.video_data is not None:
            ...
            for vid in sorted_vids:
                ...
                #tags = vid[1]["tags"]
                stats.append([video_id,video_title,video_desc,total_likes,total_dislikes,total_comments,total_views,publishedAt,total_favorite,channel_id,channel_title])
                    
            df = pd.DataFrame(stats, columns = ["video_id","video_title","video_desc","total_likes","total_dislikes","total_comments","total_views","publishedAt","total_favorite","channel_id","channel_title"])
            df.to_sql(name='Video_Data',schema='QLX-Youtube',con=engine ,if_exists='replace')        
        ...
        if self.channel_subscribers is not None:
            ...
            for sub in subscribers_data:
                ...
                stats.append([channel_id,subscriber_id,subscriber_title,subscriber_desc,customUrl,publishedAt])
                    
            df = pd.DataFrame(stats, columns = ["channel_id","subscriber_id","subscriber_title","subscriber_desc","customUrl","publishedAt"])
            df.to_sql(name='Channel_Subscribers_Data',schema='QLX-Youtube',con=engine ,if_exists='replace')
